...
runName     = '20250411flow_3';
baseDir     = fileparts(mfilename('fullpath'));

% 路径和输出目录创建统一交给 setupULMPaths
[dataDir,localPath,trackspath,savingpath,fileList,allFileNames,nBuffers] = setupULMPaths(baseDir, expName, runName);
...
ULM.numberOfParticles      = 50;
ULM.res                    = resFactor;% 超分辨因子。最终生成的超分辨图像像素比原始 IQ 图像精细 10 倍。
ULM.interpFactor           = 1 / resFactor;% 定位精度因子。在子像素定位时，将原始像素切分的粒度。
ULM.svdCutoff              = round([15, 800]);% round四舍五入到最接近的整数
ULM.max_linking_distance   = 1;% 最大连接距离
ULM.min_length             = 5;% 最小轨迹长度
...
ULM.parameters.NLocalMax = 3; % 在识别微泡中心时，算法会以一个像素为中心，检查周围 4x4 像素范围内，它是否是能量最强的点

% 皮层 ROI：cortexKeepRange 是最终保留区域，cortexProcessRange 是带缓冲的处理区域。
% 先用更宽的处理区域做局部极大值检测，再在定位函数中只保留目标区域，可减少硬截断边缘伪点。
cortexKeepRange = [130, 160];
cortexProcessMargin = 5;
cortexCandidateRange = [133, 157];
cortexProcessRange = [ ...
    max(1, cortexKeepRange(1) - cortexProcessMargin), ...
    min(nAxial, cortexKeepRange(2) + cortexProcessMargin) ...
];

ULM.parameters.LocalizationZRange = cortexKeepRange;
ULM.parameters.CandidateZRange = cortexCandidateRange;

% 滤波器配置 (Butterworth)
% ULM.butter.samplingFreq / 2 计算的是奈奎斯特频率（Nyquist Frequency），最高只能准确捕捉到的信号变化。
ULM.butter.cutoffFreq   = [30, 249];
...
%     IQ_filt(1 : 129, :, :) = 0;
%     IQ_filt(161 : end, :, :) = 0;
    % SVD 后只保留带缓冲的皮层处理区，避免在目标边界直接制造硬截断。
    IQ_filt(1:cortexProcessRange(1)-1, :, :) = 0;
    IQ_filt(cortexProcessRange(2)+1:end, :, :) = 0;
...
%     [Track_raw,Track_interp] = accela_constrain(Track_raw,Track_interp);
    ...
    fprintf('Block %03d | tracks: %4d | mean length: %5.2f frames | usage: %5.2f%%\n', ...
