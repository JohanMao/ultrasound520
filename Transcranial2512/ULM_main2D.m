...
IQfiles = fullfile(dataDir, '*.mat');    % fullfile用来自动拼接路径
fileList = dir(IQfiles);    % dir函数的作用是列出指定路径下的文件和文件夹内容
if isempty(fileList)
...
ULM.res                    = res;% 超分辨因子。最终生成的超分辨图像像素比原始 IQ 图像精细 10 倍。
ULM.interpFactor           = 1 / res;% 定位精度因子。在子像素定位时，将原始像素切分的粒度。
